...
def dataset_states_to_obs(args):
    if len(args.camera_names) == 0:
        with h5py.File(args.dataset, 'r+') as input_dataset:
            for k in input_dataset['data']['demo_0']['obs'].keys():
                if k.endswith('_image'):
                    args.camera_names.append(k[:-6])
        print('camera_names:', args.camera_names)
    ...
    # xyz_pts, rgb_pts = list(), list()
    # obs = env.reset()
    # for key in obs.keys():
    #     if key.endswith('_xyz'):
    #         cam_name = key[:-4]
    #         xyz_pts.append(obs[f'{cam_name}_xyz'].reshape([-1, 3]))
    #         rgb_pts.append(obs[f'{cam_name}_image'].reshape([-1, 3]))
    # xyz_pts = np.concatenate(xyz_pts, axis=0)
    # rgb_pts = np.concatenate(rgb_pts, axis=0)
    # meshwrite('mesh.ply', xyz_pts, rgb_pts)
    # exit()
...
"""
python diffusion_policy/scripts/robomimic_dataset_states_to_obs.py --dataset data/robomimic/datasets/transport/ph/image.hdf5 --output_name tmp.hdf5 --camera_depths --n 1 \
    --done_mode 2 --camera_height 256 --camera_width 256
